...
def main():
    ...
    #x2("^", "", 0, 0);         # error on V2
    #x2("$", "", 0, 0);         # error on V2
    #x2("\\G", "", 0, 0);       # error on V2
    #x2("\\A", "", 0, 0);       # error on V2
    #x2("\\Z", "", 0, 0);       # error on V2
    #x2("\\z", "", 0, 0);       # error on V2
    #x2("^$", "", 0, 0);        # error on V2
    x2("\\ca", "\001", 0, 1);
    #x2("\\C-b", "\002", 0, 1);  #XXX not supported
    x2("\\c\\\\", "\034", 0, 1);
    x2("q[\\c\\\\]", "q\034", 0, 2);
    #x2("", "a", 0, 0);         # fail on V2
    x2("a", "a", 0, 1);
    ...
    x3("(z)()()(?<_9>a)\\g<_9>", "zaa", 2, 3, 4);   #XXX 1 -> 4
    x2("(.)(((?<_>a)))\\k<_>", "zaa", 0, 3);
    x2("((?<name1>\\d)|(?<name2>\\w))(\\k<name1>|\\k<name2>)", "ff", 0, 2);
    #x2("(?:(?<x>)|(?<x>efg))\\k<x>", "", 0, 0);    # error on V2
    ...
    x3("\\g<_A>\\g<_A>|\\zEND(.a.)(?<_A>.b.)", "xbxyby", 3, 6, 2);  #XXX 1 -> 2
    ...
    #x2("()*\\1", "", 0, 0);            # error on V2
    #x2("(?:()|())*\\1\\2", "", 0, 0);  # error on V2
    x3("(?:\\1a|())*", "a", 0, 0, 1);
    x2("x((.)*)*x", "0x1x2x3", 1, 6);
    x2("x((.)*)*x(?i:\\1)\\Z", "0x1x2x1X2", 1, 9);
    #x2("(?:()|()|()|()|()|())*\\2\\5", "", 0, 0);  # error on V2
    x2("(?:()|()|()|(x)|()|())*\\2b\\5", "b", 0, 1);
    ...
    x2("(?s:よ.)", "よ\n", 0, 3);   #XXX m -> s
    x2("(?s:.め)", "ま\nめ", 2, 5); #XXX m -> s
    #x2("あ?", "", 0, 0);           # error on V2
    x2("変?", "化", 0, 0);
    x2("変?", "変", 0, 2);
    #x2("量*", "", 0, 0);           # error on V2
    x2("量*", "量", 0, 2);
    x2("子*", "子子子", 0, 6);
    x2("馬*", "鹿馬馬馬馬", 0, 0);
    #n("山+", "");                  # error on V2
    ...
    x2("\\x{82a0}\\x{82A2}", "あい", 0, 4)
    x2("\\p{Hiragana}\\p{Katakana}", "あイ", 0, 4)
    x2("(?ms)^A.B$", "X\nA\nB\nZ", 2, 5)
    n("(?<!(?<=a)b|c)d", "abd")
    n("(?<!(?<=a)b|c)d", "cd")
    x2("(?<!(?<=a)b|c)d", "bd", 1, 2)
