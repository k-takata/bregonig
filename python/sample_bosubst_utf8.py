...
t1 = " 横浜 045-222-1111  大阪 06-5555-6666  東京 03-1111-9999 ".encode('UTF-8')
t1p = cast(t1, c_void_p)
ctr = BoSubst(r"(\d\d)-\d{4}-\d{4}".encode('UTF-8'), r"$1-xxxx-xxxx".encode('UTF-8'), r"g8".encode('UTF-8'),
        t1p, t1p, t1p.value + len(t1),
        None, byref(rxp), msg)
if ctr > 0:
    print("after(%d)=%s" % (ctr, string_at(rxp.contents.outp).decode('UTF-8')))
    print("length=%d" % (rxp.contents.outendp - rxp.contents.outp))



ctr = BoSubst(None, r"$1-ｘｘｘｘ-\x{FF59}\x{FF59}\x{FF59}\x{FF59}".encode('UTF-8'), None,
        t1p, t1p, t1p.value + len(t1),
